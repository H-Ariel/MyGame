...
json& json::operator=(const std::initializer_list<json>& n)
{
	SetList(n);
	return *this;
}
json& json::operator=(const std::vector<json>& n)
{
	SetList(n);
	return *this;
}
json& json::operator=(const std::map<std::string, json>& n)
{
	SetDict(n);
	return *this;
}
...
void json::list_iterate(std::function<void(const json&)> func) const
{
	if (type != JsonType::List)
		THROW_TYPE_ERROR(list);
...
void json::dict_iterate(std::function<void(const std::string&, const json&)> func) const
{
	if (type != JsonType::Dictionary)
		THROW_TYPE_ERROR(dictionary);
